let ( let* ) = Result.bind
let hex text = String.to_seq text |> Seq.map (fun c -> Printf.sprintf "%02x" (Char.code c)) |> List.of_seq |> String.concat ""
...
let unhex text =
  let rec go chars acc =
    ...
    | [_] -> Error "odd hex"
    | a :: b :: tail ->
        let pair = "0x" ^ String.of_seq (List.to_seq [a; b]) in
        let* n = Option.to_result ~none:"bad hex" (int_of_string_opt pair) in
        let* byte = Option.to_result ~none:"bad hex" (List.nth_opt byte_table n) in go tail (byte :: acc)
  in go (List.of_seq (String.to_seq text)) []
...
let seeds text =
  ...
  List.fold_right (fun piece acc -> let* value = seed piece in let* rest = acc in Ok (value :: rest)) pieces (Ok [])
